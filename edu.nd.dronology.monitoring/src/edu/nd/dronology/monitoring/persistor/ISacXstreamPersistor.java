...
public class ISacXstreamPersistor implements IPersistenceManager<InfrastructureSafetyCase> {
 
	private static final XStream xstream = new XStream(new XppDriver());
	private static ILogger LOGGER = LoggerProvider.getLogger(ISacXstreamPersistor.class);

	static final boolean useCompression = false;

	@Override
	public InfrastructureSafetyCase open(InputStream fin) throws PersistenceException {
		DronologyPersistenceUtil.preprocessStream(xstream);
		SACPersistenceUtil.preprocessStream(xstream);
	...
	private InfrastructureSafetyCase loadUncompressedStream(InputStream fin) throws PersistenceException {
		try {
			...
			return (InfrastructureSafetyCase) model;
		} catch (Exception e) {
	...
	private InfrastructureSafetyCase loadedCompressedStream(InputStream fin) throws PersistenceException {
	...
	@Override
	public boolean save(InfrastructureSafetyCase o, OutputStream fout) throws PersistenceException {
		DronologyPersistenceUtil.preprocessStream(xstream);
		SACPersistenceUtil.preprocessStream(xstream);
	...
	private boolean saveUncompressedStream(InfrastructureSafetyCase o, OutputStream fout) throws PersistenceException {
	...
	private boolean saveCompressedStream(InfrastructureSafetyCase o, OutputStream fout) throws PersistenceException {
		try {
