...
/**
 * An entity used to determine the monitoring frequency of 
 * vehicles within the infrastructure.  
 * 
 * @author seanbayley
 *
 */
public class TrustManager {
	...
	private static final String PATH_TO_HISTORY = 
			Paths.get("src", "edu", "nd", "dronology", "monitoring", "trust", "history.json").toString();
	...
	public TrustManager(String pathToHistory, double forgettingFactor) {
		...
		try {
			...
			this.history = gson.fromJson(json, new TypeToken<Map<String, VehicleReputation>>(){}.getType());
	...
	public double getReputationRating(String vid) {
		// TODO: how do we want to combine reputation ratings from different assumptions?
		return 0.0;
	}
	...
	private static class InternalMonitoringEvalListener extends RemoteObject 
		...
		public void constraintEvaluated(String vid, String assumptionid, String message, Result result)
			...
			int success = 1;
			try {
				TrustManager.getInstance().constraintEvaluated(vid, assumptionid, success);
			}
