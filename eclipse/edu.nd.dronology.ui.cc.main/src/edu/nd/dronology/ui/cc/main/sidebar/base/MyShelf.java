 ...
 * <p>
 * The item children that may be added to instances of this class must be of type <code>PShelfItem</code>. <code>Control</code> children are created on the body composite of each items accessed via
 ...
 * </p>
 */
public class MyShelf extends Canvas {

	private ArrayList items = new ArrayList();
	private AbstractRenderer renderer;
	private MyShelfItem openItem;
	private MyShelfItem focusItem;
	private MyShelfItem mouseDownItem;
	private MyShelfItem hoverItem;
	 ...
	 *              </ul>
	 */
	public MyShelf(Composite parent, int style) {
		...
		this.addMouseListener(new MouseListener() {
			...
			public void mouseUp(MouseEvent e) {
				MyShelfItem item = getItem(new Point(1, e.y));
				if ((item) == null)
					return;
				// TODO remove as soon resize bug fixed
	...
	// *****ADDED CODE
	List<Listener> resizeListener = new ArrayList<>();

	public void addListener(Listener l) {
		resizeListener.add(l);

	}

	protected void triggerRefresh() {
		for (Listener l : resizeListener) {
			l.handleEvent(null);
		}
	...
	private void onPaint(GC gc) {
		...
		for (Iterator iter = items.iterator(); iter.hasNext();) {
			MyShelfItem item = (MyShelfItem) iter.next();
	...
	void createItem(MyShelfItem item, int index) {
		if (index == -1) {
			items.add(item);
		} else {
	...
	void removeItem(MyShelfItem item) {
		computeItemHeight();
		items.remove(item);
		if (openItem == item) {
			openItem = null;
			if (items.size() > 0) {
				openItem((MyShelfItem) items.get(0), false);
			}
		}
	...
	private void openItem(MyShelfItem item, boolean animation) {
		MyShelfItem previousOpen = openItem;
		openItem = item;
		focusItem = item;
		if (item == null) {
			return;
	...
	private void animateOpenFromTop(MyShelfItem previousItem, MyShelfItem newItem) {
	...
	private void animateOpenFromBottom(MyShelfItem previousItem, MyShelfItem newItem) {
		double percentOfWork = 0;
		...
		while (percentOfWork < 1) {
			...
			int yBottom = getClientArea().y + getClientArea().height
					- (itemHeight * (items.size() - (items.indexOf(newItem) + 1)));
	...
	void onResize() {
		...
		for (Iterator iter = items.iterator(); iter.hasNext();) {
			MyShelfItem item = (MyShelfItem) iter.next();
			// System.out.println("OLD SIZE:"+item.getBody().getSize());
			item.getBody().setBounds(0, 0, getClientArea().width, clientHeight);
			// System.out.println("NEW SIZE:"+item.getBody().getSize());
			// item.getBody().layout();
			item.getBodyParent().layout();
			// System.out.println("-----------");
		}
		if (openItem != null) {
			// openIt(openItem, false);
		}
		redraw();
		getDisplay().update();
	...
	private void sizeClients() {
		...
		for (int i = 0; i < items.size(); i++) {
			MyShelfItem item = (MyShelfItem) items.get(i);
	 ...
	 *              </ul>
	 */
	public MyShelfItem getItem(Point point) {
		checkWidget();
	 ...
	 *              </ul>
	 */
	public void setSelection(MyShelfItem item) {
		checkWidget();
	 ...
	 *              </ul>
	 */
	public MyShelfItem getSelection() {
		checkWidget();
		return openItem;
	}
	 ...
	 *              </ul>
	 */
	public MyShelfItem[] getItems() {
		checkWidget();
		return (MyShelfItem[]) items.toArray(new MyShelfItem[items.size()]);
	}

	/**
	 * Adds the listener to the collection of listeners who will be notified when the receiver's selection changes, by sending it one of the messages defined in the <code>SelectionListener</code>
	 * interface.
	 * <p>
	 * When <code>widgetSelected</code> is called, the item field of the event object is valid. <code>widgetDefaultSelected</code> is not called.
	 * </p>
	 * 
	 * @param listener
