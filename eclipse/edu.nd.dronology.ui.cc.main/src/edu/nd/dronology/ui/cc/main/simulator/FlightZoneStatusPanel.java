...
public class FlightZoneStatusPanel {
	 ...
	 */
	public void displayCurrentFlights(ArrayList<IFlightPlan> currentFlights, ArrayList<IFlightPlan> awaitingFlights) {
		writeText(12, true, "Current Flights", 5,rowCtr);  // Page Heading
		rowCtr +=15;
		Iterator<IFlightPlan> itr = currentFlights.iterator();  // External iterator needed due to external updates
	 ...
	 */
	public void displayPendingFlights(ArrayList<IFlightPlan> pendingFlights) {
		rowCtr +=20;
		writeText(12, true, "Pending Flights", 5,rowCtr);  // Page Heading
		rowCtr +=10;
		Iterator<IFlightPlan> itr = pendingFlights.iterator();  // External iterator needed due to external updates
		IFlightPlan flightPlan = null;
	    while(itr.hasNext()){
	    	try{
	    		flightPlan = itr.next();	
	 ...
	 */
	public void displayCompletedFlights(ArrayList<IFlightPlan> completedFlights) {
		rowCtr +=20;
		writeText(12, true, "Completed Flights", 5,rowCtr);  // Page Heading
		rowCtr +=10;
		Iterator<IFlightPlan> itr = completedFlights.iterator();  // External iterator needed due to external updates
		IFlightPlan flightPlan = null;
	    while(itr.hasNext()){
	    	try{
	    		flightPlan = itr.next();	
