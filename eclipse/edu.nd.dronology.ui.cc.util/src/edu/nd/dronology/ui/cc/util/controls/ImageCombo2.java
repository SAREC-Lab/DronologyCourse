/*******************************************************************************
 * Copyright (c) 2000, 2007 IBM Corporation and others. All rights reserved. This program and the accompanying materials are made available under the terms of the Eclipse Public License v1.0 which
 * accompanies this distribution, and is available at http://www.eclipse.org/legal/epl-v10.html
 * 
 * Contributors: IBM Corporation - initial API and implementation Tom Seidel - enhancements for image-handling Michael Edwards - further enhancements to support image AND text in collapsed state of
 * combo
 *******************************************************************************/
...
public class ImageCombo2 extends Composite {
	 ...
	 * @see Widget#getStyle
	 */
	public ImageCombo2(Composite parent, int style) {
		super(parent, style = checkStyle(style));
		setData(StyleProvider.CSS_TAG, "noskin");
		this.style = style;
		...
		GridLayoutFactory.fillDefaults().spacing(1, 0).extendedMargins(10, 0, 0, 0).numColumns(3)
				.applyTo(comboComposite);
		GridDataFactory.fillDefaults().grab(true, true).applyTo(comboComposite);

		imageLabel = new Label(comboComposite, SWT.NONE);
		//imageLabel.setData(StyleProvider.CSS_TAG, "noskin");
		GridDataFactory.fillDefaults().grab(false, true).applyTo(imageLabel);
		text = new Text(comboComposite, SWT.READ_ONLY);

		GridDataFactory.fillDefaults().grab(true, true).align(SWT.FILL, SWT.CENTER).applyTo(text);
		comboComposite.setCursor(getDisplay().getSystemCursor(SWT.CURSOR_ARROW));

		// int arrowStyle = SWT.ARROW | SWT.DOWN;
		// if ((style & SWT.FLAT) != 0) {
		// arrowStyle |= SWT.FLAT;
		// }
		arrow = new Button(comboComposite, SWT.ARROW | SWT.DOWN);
		...
		listener = new Listener() {
			...
			public void handleEvent(Event event) {
				...
				if (ImageCombo2.this == event.widget) {
					comboEvent(event);
					return;
				}
		...
		this.filter = new Listener() {
			...
			public void handleEvent(Event event) {
				...
				if (shell == ImageCombo2.this.getShell()) {
					handleFocus(SWT.FocusOut);
				}
			}
		};
	...
	public Point computeSize(int wHint, int hHint, boolean changed) {
		...
		Point th = text.computeSize(SWT.DEFAULT, SWT.DEFAULT);
	...
	Label getAssociatedLabel() {
		...
		for (int i = 0; i < siblings.length; i++) {
			if (siblings[i] == ImageCombo2.this) {
	...
	void initAccessible() {
		...
		getAccessible().addAccessibleTextListener(new AccessibleTextAdapter() {
			...
			public void getCaretOffset(AccessibleTextEvent e) {
				e.offset = ImageCombo2.this.text.getCaretPosition();
			}
		});
	...
	public void setBackground(Color color) {
		...
		if (!text.isDisposed()) {
			...
			text.getParent().setBackground(color);
	...
	public void setEnabled(boolean enabled) {
		...
		if (enabled) {
			setData(StyleProvider.CSS_TAG, "noskin");
			text.setData(StyleProvider.CSS_TAG, "noskin");
			imageLabel.setData(StyleProvider.CSS_TAG, "noskin");
			comboComposite.setData(StyleProvider.CSS_TAG, "noskin");
	...
	void setConsistent(boolean consistent) {
		if (consistent) {
			setData(StyleProvider.CSS_TAG, "cconsistent");
		}
		else{
			setData(StyleProvider.CSS_TAG, "inconsistent");
		}
	}

	public void resetSkinning() {
		setData(StyleProvider.CSS_TAG, null);
		text.setData(StyleProvider.CSS_TAG, null);
		comboComposite.setData(StyleProvider.CSS_TAG,null);
