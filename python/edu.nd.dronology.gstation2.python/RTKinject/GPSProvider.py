...
import RTCMMavlink
from GPSdriver import ubx
import threading

class GPSProvider():
	def __init__(self, port, baudrate=115200):
		self.rtcm_mavlink = RTCMMavlink.RTCMMavlink()
		self.ublox_driver = ubx.ubx(port, self.GPScallback, baudrate=baudrate)
	
	def GPScallback(self, type, data):
		if type=="gotRTCMMessage":
			self.gotRTCMData(data)
	
	def gotRTCMData(self, data):
		message = data
		self.rtcm_mavlink.RTCMDataUpdate(message)
	
	def registerNewCallback(self,callback):
		return self.rtcm_mavlink.registerNewCallback(callback)
	
	def deregisterCallback(self,callback_id):
		self.rtcm_mavlink.deregisterCallback(callback_id)
	
	def run(self):
		self.ublox_driver.run()
	
	def run_async(self):
		threading.Thread(target=self.run).start()

	def shutdown_handler(self,signal,frame):
		self.ublox_driver.shutdown_handler(signal,frame)
